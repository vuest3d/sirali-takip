...
RAPOR_DOSYASI = "index.html" # GitHub Pages için index.html olarak ayarlandı
GEMINI_API_KEY = "" # Buraya kendi API anahtarını yazabilirsin

def veri_cek():
    """Web sitesine bağlanıp Merkez listesini çeker."""
    try:
        headers = {
...
def html_sablonu_olustur(tum_veriler_json):
    """HTML şablonunu oluşturur. Veriyi JS içine gömer."""
...
<body class="bg-gray-50 min-h-screen p-4 md:p-8">
    ...
    <!-- GÖREV GEÇMİŞİ MODALI (KART TIKLAMA) -->
    ...
    <script>
        ...
        function handleFileUpload(input) {{
            ...
            reader.onload = function(e) {{
                ...
                }} catch (error) {{ alert("Dosya okunamadı: " + error); }}
            }};
        ...
        function downloadCSV() {{
            ...
            appData.forEach(record => {{
                record.liste.forEach(item => {{
                    let cleanItem = item.replace(/"/g, '""');
                    csvContent += `"${{record.tarih}}","${{cleanItem}}"\\n`;
                }});
            }});
        ...
        function downloadJSON() {{
            saveFile(new Blob([JSON.stringify(appData, null, 4)], {{ type: 'application/json' }}), "dagitim_yedek.json");
        }}
        ...
        function toggleResponseArea(show) {{
            const el = document.getElementById('ai-response-container');
            if (show) {{ el.classList.remove('hidden'); document.getElementById('ai-loader').classList.remove('hidden'); document.getElementById('ai-result').innerHTML = ''; }} 
            else {{ document.getElementById('ai-loader').classList.add('hidden'); }}
        }}
...
def rapor_olustur(gecmis_veriler):
    ...
    # Veriyi JSON string'e çevirip HTML içine gömüyoruz
